...
#define PYRAMID_BASE    (100)

int pyramid[PYRAMID_BASE][PYRAMID_BASE];

int main(void){
    int32_t i, j;
    uint32_t left, right;
    uint32_t buf[PYRAMID_BASE][PYRAMID_BASE];
    bool leftPath[PYRAMID_BASE - 1][PYRAMID_BASE - 1];

    clock_t begin, end;
    begin = clock();

    memset(buf, 0, sizeof(buf));
    memset(leftPath, false, sizeof(leftPath));

    //Determine the best path by working from the base up.
    //The max number possible is stored in buf and the
    //left\right path is stored in leftPath.
    //Left path tells us if we search left of right as we go down the triangle
    //for the max path.
    for(i = (PYRAMID_BASE - 2); i >= 0; i--){
        for(j = 0; (j < (PYRAMID_BASE -1)) && (pyramid[i][j] != 0); j++){
            left = pyramid[i + 1][j] + buf[i +1][j];
            right = pyramid[i + 1][j + 1] + buf[i + 1][j + 1];
            if(left > right){
                leftPath[i][j] = true;
                buf[i][j] = left;
            }else{
    ...
    uint64_t total = pyramid[0][0];
    int32_t pos = 0;
    int32_t m, n;
    for(m = 0; m < (PYRAMID_BASE - 1); m++){
        if(leftPath[m][pos] == true){
            total += pyramid[m + 1][pos];
        }else{
            pos += 1;
            total += pyramid[m + 1][pos];
        }
